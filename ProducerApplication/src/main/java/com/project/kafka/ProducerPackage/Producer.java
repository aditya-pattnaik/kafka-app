...
public class Producer {
	...
	public static void main(String[] args) throws InterruptedException, FileNotFoundException {
		...
		while (streamInput.hasNextLine()) {
			...
			// message written to kafka partition will be typically in <key, Value>
			// structure.
			// args[1] - Topic name.
			// columns[0] - first field <CustomerID> is used as Key.
			// Record - complete record as value.
			final ProducerRecord<String, String> record = new ProducerRecord<String, String>(args[1], columns[0],
					Record);
